...
fit_ff2 <- function(B_hat, S_hat, N, R, kmax=100, zero_thresh = 1e-15, adjust=TRUE){
  ...
  if(all(Sig_new ==0)){
    ...
    # Fit fixed factors first
    f1 <- flash.init(B_tilde, S = sqrt(lambda_min), var.type = 2) %>%
          flash.init.factors(., EF = list(A_rand, W),
                             prior.family = prior.normal(scale= 1, g_init=gg, fix_g = TRUE)) %>%
          flash.fix.loadings(., kset = 1:(ntrait-1), mode=2) %>%
          flash.backfit() %>%
          flash.add.greedy(Kmax = kmax, init.fn = init.fn.default)
    n1 <- f1$n.factors


    f2 <- f1 %>% flash.backfit(kset = (ntrait):n1)

    F_hat <- f2$loadings.pm[[2]][, (ntrait):n1]
    L_hat <- f2$loadings.pm[[1]][,(ntrait):n1]

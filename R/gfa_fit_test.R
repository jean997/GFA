...
#'@export
gfa_fit_test <- function(Z_hat, B_std, N, R, params = gfa_default_parameters(),
                    no_wrapup = FALSE){
  ...
  W <-eS$vectors %*% diag(sqrt(vals))
  nf <- ncol(W) # Number of fixed factors
  ...
  fit <-  flash_init(data = Y, S = 0, var_type = 2) %>%
    flash_greedy(Kmax = params$kmax,
                  init_fn = params$init_fn,
                  ebnm_fn = list(params$ebnm_fn_L, params$ebnm_fn_F) )

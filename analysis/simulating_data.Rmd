...
From the expression for $\Sigma$ we can see that one of the components of $\Sigma$ is $FF^\top$ which is the genetic covariance. Will we be able to distinguish low rank structure from correlation due to sample overlap. Here are some illustrative examples. 


1. $\Sigma = \Sigma_G = \frac{1}{N} F F^\top + \sigma^2_e I$ and $L$ is dense. 

Under this condition, it should be impossible to distinguish the low rank structure of the summary statistics from covariance due to sample overlap. This occurs when

+ All traits have the same heritability
+ All heritability is mediated by factors, or the same proportion of each trait's heritability is meadiated by factors
+ Either all factors are fully heritable or have the same heritability
+ Non-genetic correlation between traits is equal to $I$. 
...
where both $L$ and $B$ are dense and normally distributed and elements of $E^\star$ are iid $N(0,1)$. In this case performing the eigenvector correction completely ablates the signal of the low rank structure so we should not find any factors after performing it. 
To make the point, I will set $N$ very high so we know that lack of power is not a problem. 
...
dat <- sim_sumstats_lf(F_mat = F_mat, N = 100000, J = 5000, 
                       h_2_trait = rep(0.3, 20), omega = rep(1, 20), h_2_factor = rep(1, 5), 
                      pi_L = rep(1, 5), pi_theta = 0, R_E = diag(rep(1, 20)))

#all.equal(dat$Sigma*10000, dat$F_mat%*%t(dat$F_mat))
```

Confirm that FLASH does not find any factors in this data after transforming by eigenvectors. The naive fit recovers the factors close to perfectly. 
...
1. $\Sigma = \Sigma_G = \frac{1}{N} F F^\top + \sigma^2_e I$ and $L$ is sparse. 

The same circumstance as above but with sparse $L$. Note that we would get the same pattern if $L$ were a sum of a dense effect and a sparse effect. I will decrease $\pi_L$ to 0.1 and increase the number of SNPs by a factor of 10 so there are the same number of effect variants. 
...
Consequences: We will be unable to detect dense factors for traits measured in the same sample because the signal is confounded with the pattern of the noise. We generally don't expect factors to be completely dense (i.e. variant effects normally distributed with a single variance for all SNPs). One exception to this is that "population stratification" factors may be dense. 

The eigenvector transformation can be thought of both as a correction for sample overlap and as removing dense factors. In this sense, it may be wise to use it even when samples don't overlap as a correction for population stratification effects. 

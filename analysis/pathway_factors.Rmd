...
```{r, eval=FALSE, cache=TRUE, include=FALSE, fit1}
#dat <- list(data=data, LL = LL , fixl=fixl)
#saveRDS(dat, file="analysis_data/pathway_loadings_data.RDS")

data <- flash_set_data(Y=mats$beta_hat, S = mats$se_hat)
f6 <- flash_add_fixed_loadings(data=data, LL=LL, fixl=fixl, var_type="zero")
f6 <- flash_add_factors_from_data(data, K=30, f_init=f6, var_type="zero")
f6 <- flash_backfit(data, f6, var_type="zero")
```

```{r, eval=FALSE, plot1}
ldf <- flash_get_ldf(f6, drop_zero_factors = FALSE)
plot_factors(ldf$f, traits, factor_names = )
```

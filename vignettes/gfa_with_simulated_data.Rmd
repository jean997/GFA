...
dat <- sim_sumstats_lf(F_mat = myF, N = 50000, J = 10000, h_2_trait = rep(0.5, 3),
                       ...
                       #omega = rowSums(myF^2)*0.8/1.134, pi_L = rep(0.1,2), pi_theta = 0.1, 
                       overlap_prop =  0)
names(dat)
```
...
fit_plain <- fit_ff_update(Z_hat = Z_hat)
```

```{r}
fit_withR <- fit_ff_update(Z_hat = Z_hat, R = R)
```
...
Rpt <- with(dat, est_R_pairwise(beta_hat, se_beta_hat, N = rep(N, 50), z_scores = TRUE, p_val_thresh = 0.05))
```
...
```{r}
fit1 <- fit_ff_update(Z_hat = Z_hat_indep, R = R)

fit2 <- fit_ff_update(Z_hat = Z_hat_indep, R = Rpt)
```
...
We used a set of independent variants to estimate the factors in the previous section. We might be interested in estimating $L$, the variant-factor effect matrix in all of the variants. We can do this post-hoc using the `est_L_flash2` function. 

```{r}

ft <- est_L_flash2(Z_hat = Z_hat_indep, fit = fit1$fit, tol = 1e-7)

fit10k <- est_L_flash2(Z_hat = Z_hat[1:10000,], fit = fit2$fit)

fit500 <- est_L_flash2(Z_hat = Z_hat[1:500,], fit = fit1$fit)
```

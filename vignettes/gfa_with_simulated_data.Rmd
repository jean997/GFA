...
This toy example is designed to have a very strong signal that can be seen visually, but we don't consider these parameters realistic. The heritability of each trait is 0.5, $N = 50,000$, $\omega = 0.8$ for each trait (most heritability is mediated by factors), the sample size is 50k and we generate 10k independent SNPs. $L$ and $\Theta$ both have 10% non-zero elements. Becuase we set the proportion of overlap to 0, we omit `h_2_factor` and `R_E`. We could optionally have also supplied minor allele frequencies. With this argument missing, SNPs are assumed standardized to variance 1. 

```{r}

dat <- simGWAS::sim_lf(F_mat = myF, N = 50000, J = 10000, h2_trait = rep(0.5, 3),
...
This is accomplished via the function `fit_ff_update` (function name may change in the future). This function has a lot of options but can be fit at minimum with either 
...
If the argument `R` is omitted, GFA will assume that there is no sample overlap and provide a warning. These options are enough to fit most data sets. We first fit without `R` and then with. The results should be very similar because our estimate is so close to the identity. 


```{r}
Z_hat = with(dat, beta_hat/se_beta_hat)
fit_plain <- gfa_fit(Z_hat = Z_hat)
```

```{r}
fit_withR <- gfa_fit(Z_hat = Z_hat, R = R)
```
Now plotting results:

```{r}
fit_plain$F_hat %>%
  plot_factors(., col_names = c("T3", "T2", "T1")) +
  scale_fill_gradient2(low = viridis(3)[1], high = viridis(3)[2], name = "Normalized\nEffect") +
  xlab("Mediator") +
  ylab("Trait") +
...
```
We get nearly identical factors using either the method accounting for correlation or the the method not accounting for correlation. This is good, since the data were created with no correlation. 

The factors capture the truth quite well. Note that our estimated factors are in a different order than the original. It is important to keep in mind that the ordering of the factors is arbitrary.  We also have a third factor that effects (almost) only trait 2. This occurs sometimes, especially when GWAS signal is very strong, but does not represent incorrect information. There are variants that effect only trait 2 and our model is capturing these through the third. The $\Theta$ term also captures variation that affects individual traits. 
...
# specify complete sample overlap
N <- matrix(50000, nrow = ntrait, ncol = ntrait)
...
dat <-  sim_lf(F_mat = myF, N=N, J=nvar, h2_trait = h2_trait,
...
Rpt <- with(dat, est_R_pairwise(beta_hat, se_beta_hat, N = diag(N), z_scores = TRUE, p_val_thresh = 0.05))
```
...
plot_factors(Rpt) + theme(axis.title =  element_blank()) + ggtitle("Estimated R")
plot(dat$R, Rpt, xlab = "True Correlation", ylab = "Estimated Correlation")
abline(0, 1)
```


```{r}
plot(dat$R, Rpt, xlab = "True Correlation", ylab = "Estimated Correlation - Threshold method")
abline(0, 1)
```
...
```{r}
ix <- sim_ld_prune(dat, R_LD = ld_mat_list, r2_thresh = 0.01)
...
```{r}
#fit1 <-gfa_fit(Z_hat = Z_hat_indep, R = R)

fit2 <- gfa_fit(Z_hat = Z_hat_indep, R = Rpt)

fit0 <- gfa_fit(Z_hat  = Z_hat_indep)

fit_o <- gfa_fit(Z_hat  = Z_hat_indep, R = dat$R)
```
...
```{r}
sol0 <- min_norm(f_true = dat$F_mat, f_hat = fit0$F_hat, return_Q  = TRUE)
sol_o <- min_norm(f_true = dat$F_mat, f_hat = fit_o$F_hat, return_Q  = TRUE)
#sol_1 <- min_norm(f_true = dat$F_mat, f_hat = fit1$F_hat, return_Q  = TRUE)
sol_2 <- min_norm(f_true = dat$F_mat, f_hat = fit2$F_hat, return_Q  = TRUE)
p_o <- sol_o$best_est %>%
  plot_factors(.) +
  scale_fill_gradient2(low = viridis(3)[1], high = viridis(3)[2], name = "Normalized\nEffect") +
  xlab("Mediator") +
  ylab("Trait") +
  #ggtitle("GFA Estimate") +
  #ggtitle("EBMF Estimate") +
  #scale_x_discrete(position = "top")+
  theme(axis.text.x = element_text(angle = 0),
           panel.background = element_rect(fill = "white"),
           axis.ticks = element_blank(), 
...
#  scale_fill_gradient2(low = viridis(3)[1], high = viridis(3)[2], name = "Normalized\nEffect") +
  xlab("Trait") +
  ylab("Trait") +
  #ggtitle("Environmental Correlation") +
  #scale_x_discrete(position = "top")+
  theme(axis.text.x = element_text(angle = 0),
           panel.background = element_rect(fill = "white"),
           axis.ticks = element_blank(), 
...
```{r}
disc(fit1$F_hat, dat$F_mat, lambda = 0)
disc(fit2$F_hat, dat$F_mat, lambda = 0)
```
...
```{r}
fit_noR <- fit_ff_update(Z_hat = Z_hat_indep)
```
...
Without including R, we generate many factors that don't correspond to the true structure of the effects and instead correspond in full or in part to the patterns of environmental correlation. 
...
## Estimating $L$ in left out variants

We used a set of independent variants to estimate the factors in the previous section. We might be interested in estimating $L$, the variant-factor effect matrix in all of the variants. We can do this post-hoc using the `gfa_estL_posthoc` function. 

```{r}

ft <- gfa_estL_posthoc(Y = Z_hat_indep, fit = fit2, tol = 1e-7)

fit10k <- gfa_estL_posthoc(Y = Z_hat[1:10000,], fit = fit2)

fit500 <- gfa_estL_posthoc(Y = Z_hat[1:500,], fit = fit2)
```

...
Next we simulate some data using the `sim_lf` function in `GWASBrewer` which simulates from the model
...
To fit GFA to our toy data, we use the `gfa_fit` function. `gfa_fit` can take two types of input, either z-scores and sample size or effect estimates and standard errors. If you know the sample size for each GWAS, it is better to use z-scores and sample size. In either case, GFA will be fit using z-scores and the fitted factors will be re-scaled so that they are on the standardized trait scale. If z-scores are provided and no sample size is given, factors will still be estimated but will be left on the z-score scale. We will discuss scaling in greater detail in the next example.
...
- `F_hat_est`: estimated factors on the estimated (z-score) scale.
...
# prorportion of effect variants
pi_L <- pi_theta <- 1000/nvar
```

Next we generate a factor structure:
...
Finally, since GWAS samples overlap, we can provide the environmental correlation of the traits. This is the correlation of the environmental component of the trait not mediated by the factors. We will use a block correlation structure.
```{r}
...
dat <-  sim_lf(F_mat = myF, N=Nmat, J=nvar, h2_trait = h2_trait,
            omega = omega, h2_factor = h2_factor,
            pi_L = rep(pi_L, nfactor), pi_theta = pi_theta,
            R_E = R_E, 
            R_LD = ld_mat_list, af = AF, 
            est_s = TRUE) 
dat <- GWASBrewer:::calc_ld_scores(dat, ld_mat_list)
```
...
R2 <- with(dat, R_pt(beta_hat, s_estimate, p_val_thresh = 0.05)) 
```

We can take a look at the estimated $R$ and the true $R$. The LD-score method gives a more accurate estimate but takes longer to compute.
...
plot(dat$R, R2, xlab = "True Correlation", ylab = "Estimated Correlation - p threshold")
abline(0, 1)
plot(dat$R, R1, xlab = "True Correlation", ylab = "Estimated Correlation - LDSC")
abline(0, 1)
```

Next, we need to obtain an independent (LD-pruned) set of variants. We can use `GWASBrewer` to find this for the simulated data set.
...
Finally, we can fit GFA with or without adjustment for sample ovelap provided by the `R` argument. Recall that `R1` was estimated using LD-score regression.
...
```{r}
min_norm(f_true = dat$F_mat, f_hat = fit1_ff$F_hat)$solution
```
...
If we compare these results with the results where we omitted the `R` argument, we can see that not adjusting for the nuisance correlation has resulted in thre extra factors being estimated that don't match any true factors. 

```{r}
min_norm(f_true = dat$F_mat, f_hat = fit0$F_hat)$solution
```
If we plot the factors estimated with and without the correction for overlapping samples, we can see that the factor structure of the "extra" factors mimics the structure of the environmental correlation. We can also see that some of the "matching" factors estimated in `fit0` are also contaminated with some of the structure of the environmental correlation.
```{r}
p0 <- fit0$F_hat %>% 
  plot_factors(.) +
